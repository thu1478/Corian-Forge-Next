...
function passiveFromRules(rules: RulesWithCharges, traitId: string): ChargeDefinition | undefined {
    const global = asChargeDef(rules.passives?.[traitId])
    if (global) return global

    for (const cls of Object.values(rules.classes || {})) {
        const passives = (cls as { passives?: Record<string, unknown> })?.passives
        const p = asChargeDef(passives?.[traitId])
        if (p) return p
    }

    for (const race of Object.values(rules.races || {})) {
        const passives = (race as { passives?: Record<string, unknown> })?.passives
        const p = asChargeDef(passives?.[traitId])
        if (p) return p
    }

    const feat = asChargeDef(rules.system?.feats?.[traitId])
    if (feat) return feat
...
>(
    ...
    return { ...prev, traits, actions, reactions }
}
