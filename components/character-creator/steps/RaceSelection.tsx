...
export function RaceSelection({
  ...
  const ALL_RACES = useMemo(() => {
    return Object.entries(rulesData.races as any).reduce(
      (acc, [rId, r]) => {
        const race = r as any;
        return {
          ...acc,
  ...
  const selectedIdsForRace = (rId: string) =>
    raceId === rId ? racialTraits.map((t) => t.id) : [];

  const calculatePoints = (rId: string) => {
    const race = ALL_RACES[rId];
    const ids = selectedIdsForRace(rId);
    return (
      race?.passives
  ...
  const needsEffectChoice = (p: RacialPassive & { selectAmount?: number; effects?: any[] }) =>
    typeof p.selectAmount === "number" &&
    p.selectAmount > 0 &&
    Array.isArray(p.effects) &&
    p.effects.length > p.selectAmount;
    ...
    <div className="w-full max-w-7xl mx-auto px-8 py-8 min-h-screen flex flex-col text-slate-900 dark:text-slate-100">
      {expandedRaceId ? (
        <div className="flex flex-col h-full animate-in slide-in-from-right-4 duration-300">
          <button
            onClick={() => setExpandedRaceId(null)}
            className="flex items-center gap-2 text-slate-500 hover:text-slate-900 dark:hover:text-white mb-6 group"
          >
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-[2rem] p-8 lg:p-12 mb-6 shadow-xl">
              ...
              <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800/50 rounded-xl p-4 text-center min-w-[100px]">
                ...
                <div
                  className={`text-2xl font-black tabular-nums ${
                    calculatePoints(expandedRaceId) === 3 ? "text-green-600" : "text-amber-600"
                  }`}
                >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
              <section>
                <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-6 border-l-2 border-purple-500 pl-3">
                  Innate Traits
                </h3>
                <div className="grid gap-3">
                  {ALL_RACES[expandedRaceId].passives
                    .filter((p) => p.type === "innate")
                    .map((p) => (
                      <div
                        key={p.uid}
              ...
              <section>
                ...
                <div className="grid gap-2">
                  {ALL_RACES[expandedRaceId].passives
                    .filter((p) => p.type === "selectable")
                    .map((p) => {
                      const active =
                        raceId === expandedRaceId &&
                        racialTraits.some((t) => t.id === p.id);
                      const choice = needsEffectChoice(p as any);
                      const selectedRef = racialTraits.find((t) => t.id === p.id);
                      const pickedIdx = selectedRef?.selectedEffectIndices?.[0];

                      const handleCardClick = () => {
                        if (raceId !== expandedRaceId) onSelectRace(expandedRaceId);
                        if (choice) {
                          if (active) onToggleSelectable(p.id);
                          else
                            onToggleSelectable(p.id, {
                              selectedEffectIndices: [0],
                            });
                        } else {
                          onToggleSelectable(p.id);
                        }
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-start gap-4">
                            <div
                              className={`mt-0.5 w-5 h-5 rounded border flex items-center justify-center shrink-0 pointer-events-none ${
                                active
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex justify-between items-center mb-0.5 gap-2">
                                ...
                                <span className="text-[10px] font-black text-amber-600 bg-amber-50 dark:bg-amber-900/30 px-2 py-0.5 rounded italic shrink-0">
                                  [{p.ptCost} PTS]
                                </span>
                              </div>
                                      ...
                                      <button
                                        ...
                                        className={`text-xs font-bold px-3 py-2 rounded-lg border transition-colors ${
                                          on
                                            ...
                                            : "bg-slate-100 dark:bg-slate-800 border-slate-300 dark:border-slate-600 text-slate-800 dark:text-slate-100 hover:bg-slate-200 dark:hover:bg-slate-700"
                                        }`}
                                      >
        ...
        <>
          <div className="mb-10">
            ...
            <p className="text-slate-500 dark:text-slate-400 text-lg font-medium">
              Define your character&apos;s foundational lineage.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-10">
            {Object.entries(ALL_RACES).map(([rId, race]) => {
              ...
              return (
                <div
                  key={rId}
                  className={`p-8 rounded-[2.5rem] border-2 transition-all duration-300 flex flex-col ${
                    isSelected
                      ? "bg-white dark:bg-slate-900 border-purple-500 shadow-xl ring-4 ring-purple-500/5"
                ...
                >
                    ...
                    <div className="bg-slate-100 dark:bg-slate-950 border border-slate-200 rounded-xl px-4 py-2 text-center min-w-[70px]">
                      <div
                        className={`text-lg font-black tabular-nums ${
                          isSelected && pts === 3 ? "text-green-600" : "text-amber-600"
                        }`}
                      >
                        {isSelected ? pts : 0}/3
                      </div>
                      ...
                      {race.passives
                        ...
                        .map((p) => (
                          <span
                            key={p.uid}
                            title={p.description}
                            className="text-xs px-2 py-1 rounded border border-amber-300 dark:border-amber-700"
                          >
                            {p.name} ({p.ptCost ?? 0})
                          </span>
                        ))}
                  ...
                  <div className="flex gap-4 mt-auto pt-6">
                    <button
                      type="button"
                      onClick={() => setExpandedRaceId(rId)}
                      className="flex-1 py-4 rounded-xl font-black text-xs uppercase tracking-widest bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 transition-all"
                    >
                      Details
                    </button>
                    {isSelected && pts === 3 && (
                      <button
                        type="button"
                        onClick={onNext}
                        className="flex-1 py-4 rounded-xl font-black text-xs uppercase tracking-widest bg-purple-600 text-white shadow-md transition-all"
                      >
                        Accept
                      </button>
                    )}
