...
export function AbilityScores({
  ...
  const calculatePointsUsed = () => {
    return Object.values(scores).reduce((total, score) => {
      return total + (rules.system.pointBuy[score] || 0);
    }, 0);
  };
  ...
  const handleIncrement = (ability: Attribute) => {
    ...
    const currentCost = rules.system.pointBuy[currentScore] || 0;
    const nextCost = rules.system.pointBuy[currentScore + 1] || 0;
  ...
  return (
    <div className="flex flex-col h-full animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4">
        ...
        <div className="bg-gray-900 border border-gray-700 rounded-xl p-4 flex items-center gap-4 shadow-inner">
          <div className="text-gray-400 text-sm uppercase tracking-wider font-bold">
            Points Remaining
          </div>
          <div
            className={`text-3xl font-black ${pointsRemaining === 0 ? 'text-green-400' : 'text-amber-400'}`}>
      ...
      <div className="bg-gray-800/50 border border-gray-700 rounded-xl overflow-hidden mb-8">
        <div className="grid grid-cols-[2fr_1fr_1fr_1fr_1fr] gap-4 p-4 border-b border-gray-700/50 bg-gray-900/50 text-xs uppercase tracking-wider font-bold text-gray-500">
          <div>Attribute</div>
        ...
        <div className="divide-y divide-gray-700/50">
          {ATTRIBUTES.map((attr) => {
            ...
            const currentCost = rules.system.pointBuy[baseScore] || 0;
            const nextCost = rules.system.pointBuy[baseScore + 1] || 0;
            ...
            return (
              <div
                key={attr.id}
                className="grid grid-cols-[2fr_1fr_1fr_1fr_1fr] gap-4 p-4 items-center hover:bg-gray-800/50 transition-colors">
                
                <div>
                  <div className="font-bold text-gray-200">{attr.name}</div>
                ...
                <div className="flex items-center justify-center gap-2">
                  <button
                    ...
                    className="w-6 h-6 rounded-full bg-gray-700 flex items-center justify-center text-gray-300 hover:bg-gray-600 disabled:opacity-30 disabled:cursor-not-allowed transition-colors">
                    
                    <MinusIcon className="w-3 h-3" />
                  </button>
                  <span className="w-6 text-center font-bold text-gray-300">
                    {baseScore}
                  </span>
                  <button
                ...
                <div className="text-center">
                  <span className="inline-block px-3 py-1 bg-purple-900/30 text-purple-300 border border-purple-500/30 rounded-lg font-bold">
                    {modifier}
                  </span>
                </div>
              </div>);
      ...
      <div className="mb-8">
          ...
          <div
            ...
            className="bg-gray-900 border border-gray-700 rounded-xl p-4">
              ...
              className="w-full bg-gray-800 border border-gray-600 rounded-lg px-3 py-2 text-white text-sm focus:ring-purple-500 focus:border-purple-500 outline-none">
      ...
      <div className="flex justify-between mt-auto">
        ...
        <button
          ...
          className={`flex items-center gap-2 px-6 py-3 rounded-lg font-bold transition-all ${allBonusesAssigned ? 'bg-purple-600 hover:bg-purple-500 text-white shadow-lg shadow-purple-900/30' : 'bg-gray-800 text-gray-500 cursor-not-allowed'}`}>
