...
import { X, GripHorizontal, Shield, Sparkles, Swords } from "lucide-react"
...
export function DamageCalculator({
  ...
  const relevantDefense = damageType === "physical" ? defense : stability
  const calculatedDamage = Math.max(0, rawDamage - relevantDefense)
        ...
        <div className="p-4 bg-muted/30 rounded-lg border border-border space-y-3">
          ...
          <div className="flex items-center justify-between text-sm">
            <span className="text-muted-foreground">
              {damageType === "physical" ? "Defense" : "Stability"}:
            </span>
            <span className="font-mono font-bold text-foreground text-base">-{relevantDefense}</span>
          </div>
          <div className="border-t border-border pt-3">
            <div className="flex items-center justify-between">
              ...
              <span className={cn(
                ...
                calculatedDamage > 0 ? "text-red-600 dark:text-red-400" : "text-emerald-600 dark:text-emerald-400"
              )}>
                {calculatedDamage}
              </span>
            </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="p-3 bg-cyan-100/50 dark:bg-cyan-950/30 rounded-lg border border-cyan-300 dark:border-cyan-700/50">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-xs text-cyan-700 dark:text-cyan-300 uppercase tracking-wider">Barrier</span>
            </div>
            <span className="font-mono font-bold text-cyan-700 dark:text-cyan-300 text-lg">
              {barrier.current}/{barrier.max}
            </span>
          </div>
          <div className="p-3 bg-red-100/50 dark:bg-red-950/30 rounded-lg border border-red-300 dark:border-red-700/50">
            <div className="flex items-center gap-2 mb-1">
              <Shield className="w-4 h-4 text-red-600 dark:text-red-400" />
              <span className="text-xs text-red-700 dark:text-red-300 uppercase tracking-wider">HP</span>
            </div>
            <span className="font-mono font-bold text-red-700 dark:text-red-300 text-lg">
              {hp.current}/{hp.max}
            </span>
          </div>
        </div>
