...
export function DamageCalculator({
    ...
    const [sundered, setSundered] = useState(false)
            ...
            <div className="p-4 space-y-4">
                <div className="space-y-2">
                    ...
                    <p className="text-[11px] text-muted-foreground leading-snug">
                        Channel sets defense &amp; penetration. Damage type (below) is for resistances and
                ...
                <div className="space-y-2">
                    ...
                    <div
                        role="radiogroup"
                        aria-label="Damage type for resistances and vulnerabilities"
                    ...
                    >
                        {damageTypes.map((t) => (
                            <button
                                ...
                                className={cn(
                                    "rounded-lg border-2 px-2.5 py-2 text-xs font-medium capitalize transition-all sm:px-3 sm:text-sm",
                                    damageType === t
                ...
                {usesDefense && (
                    <div className="space-y-3">
                        <div className="space-y-2">
                            <label className="text-sm font-semibold text-foreground">Sundered</label>
                            <button
                                type="button"
                                onClick={() => setSundered((v) => !v)}
                                className={cn(
                                    "w-full flex items-center justify-center gap-2 px-3 py-3 rounded-lg border-2 transition-all text-sm font-medium",
                                    sundered
                                        ? "bg-amber-100 border-amber-500 text-amber-900 dark:bg-amber-950/50 dark:border-amber-600 dark:text-amber-200"
                                        : "bg-muted/30 border-border text-muted-foreground hover:bg-muted/50"
                                )}
                            >
                                <Shield className="w-4 h-4 shrink-0"/>
                                {sundered ? "Sundered (defense ÷2, round up)" : "Sundered off"}
                            </button>
                            <p className="text-[11px] text-muted-foreground leading-snug">
                                When on, defense counts as half (round up), then penetration subtracts as usual.
                            </p>
                        </div>
                        <div className="space-y-2">
                ...
                <div className="grid grid-cols-2 gap-3">
                    <div
                        ...
                        <div className="font-mono font-bold text-lg flex items-baseline gap-2">
                            <span
                                className="line-through text-xs opacity-50 text-cyan-900 dark:text-cyan-100">{barrier}</span>
                    ...
                    <div
                        ...
                        <div className="font-mono font-bold text-lg flex items-baseline gap-2">
                            <span
                                className="line-through text-xs opacity-50 text-red-900 dark:text-red-100">{hp.current}</span>
                            <span
                                className={cn(
