...
import { parseWeaponBaseDamage } from "@/logic/equipment/weapon-utils"
...
/** Human-readable armor defense (base + optional attribute modifier cap). */
export function formatArmorDefenseValue(defense: ArmorItem["defense"]): string {
  ...
  if (defense?.attribute && typeof defense.attrMax === "number") {
    return `${v} + ${fmtAttrName(defense.attribute)} modifier (max +${defense.attrMax})`
  }
  if (defense?.attribute) {
    if (v === 0) return `${fmtAttrName(defense.attribute)} modifier`
    return `${v} + ${fmtAttrName(defense.attribute)} modifier`
  }
  return String(v)
}
...
export function equipmentStatSummaryFromDef(def: Record<string, unknown>): string | null {
  ...
  if (t === "armor" && def.defense != null && typeof def.defense === "object") {
    ...
    if (typeof d.attribute === "string" && d.attribute) {
      if (v === 0) {
        defStr = `${fmtAttrName(d.attribute)} modifier`
      } else {
        defStr += ` + ${fmtAttrName(d.attribute)} modifier`
      }
