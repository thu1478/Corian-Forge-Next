...
import { isAnimaWeaponSlotUid } from "@/logic/equipment/anima-weapon-slots"
import { resolveAnimaHandWeapon, type RulesWithNaturalWeapons } from "@/logic/equipment/natural-weapons"
...
function weaponAttributesCompatible(
    w: WeaponItem,
    rollStats: readonly string[],
    actionTags: readonly string[]
): boolean {
    if (rollStats.length === 0) return true
    // Throwing attacks use a throwing weapon's damage even when roll stats differ (e.g. Throw + handaxe).
    if (
        actionTagsIncludeCanonical(actionTags, "Throwing") &&
        actionTagsIncludeCanonical(w.tags, "Throwing")
    ) {
        return true
    }
...
function weaponMatchesActionKind(w: WeaponItem, actionTags: string[]): boolean {
    ...
    const wBrawling = actionTagsIncludeCanonical(wTags, "brawling")
    const actionThrowing = actionTagsIncludeCanonical(actionTags, "Throwing")
    const actionBrawling = actionTagsIncludeCanonical(actionTags, "brawling")
    const satisfiesRanged = wRanged || (actionThrowing && wThrowing)

    // Brawling breaks melee/ranged (e.g. Force Palm + gloves). Throwing breaks it for thrown attacks.
    if (wantsRanged && actionBrawling && wBrawling && wMelee) return true
    if (wantsRanged && actionThrowing && wThrowing) return true
...
function weaponPassesActionChecks(
    ...
    } else if (!weaponAttributesCompatible(w, rollStats, actionTags)) {
        return false
    }
...
export function hasThrowingWeaponInHands(
    activeWeapon: InventoryItem | null | undefined,
    offhandWeapon: InventoryItem | null | undefined
): boolean {
    for (const w of [activeWeapon, offhandWeapon]) {
        if (!isWeaponItem(w)) continue
        if (actionTagsIncludeCanonical(w.tags || [], "Throwing")) return true
    }
    return false
}
...
/**
 * Equipment-granted actions: use the granting item's weapon stats only (no fallback to other hand weapons).
 */
export function resolveWeaponForGrantedEquipmentAction(
    grantingItem: InventoryItem | null | undefined,
    actionTags: string[] | undefined,
    rollStats: readonly string[] | undefined,
    options?: WeaponActionEligibilityOptions
): WeaponItem | null {
    if (!grantingItem || !isWeaponItem(grantingItem)) return null
    ...
    if (weaponPassesActionChecks(grantingItem, tags, stats, firearmAction, tightGrip)) {
        return grantingItem
    }

    // Trust item↔action pairing from catalog when checks fail on edge tags.
    return grantingItem
}
